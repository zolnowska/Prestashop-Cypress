...
describe('Homepage: ', () => {
  ...
  it('User clicks button "Sign in".', () => {
    ...
    cy.contains(loginIn.createAccountBtn, 'No account? Create one here').click()
    cy.url().should('include', 'create_account')
  })

  it('Button "Basket(0)" is displayed. Its inactive and doesnt have any product inside.', () => {
    cy.get(topMenu.basketBtn).should('be.visible').and('have.class', 'inactive')
    cy.get(topMenu.basketText).should('be.visible').contains('Basket')
    cy.get(topMenu.basketproductsCountText).should('be.visible').contains('(0)')
  })
